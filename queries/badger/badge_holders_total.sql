...
(
	...
	AND tokenid = t.tokenid) AS "In", COALESCE((SELECT SUM(erc1155_value)
	FROM polygon.core.ez_nft_transfers
	WHERE nft_address = t.nft_address
...
(
	...
	SELECT  nft_address
	       ,NULL                           AS tokenid
	       ,COUNT(DISTINCT nft_to_address) AS "Total Holders"
	FROM balances
	WHERE nft_to_address != '0x0000000000000000000000000000000000000000'
	GROUP BY  nft_address
	ORDER BY tokenid
